...
BEGIN
    ...
    RETURN QUERY SELECT id, admin_level, country_code, indexOf(country_code_rankings, country_code), name || '--0', name, NULL::varchar, population, point as subquery1 FROM appfd_place WHERE name = ANY(names);
    RETURN QUERY SELECT appfd_place.id, appfd_place.admin_level, country_code, indexOf(country_code_rankings, country_code), appfd_alias.alias || '--0', appfd_place.name, appfd_alias.alias, appfd_place.population, appfd_place.point as subquery2 FROM appfd_place INNER JOIN appfd_aliasplace on (appfd_place.id = appfd_aliasplace.place_id) INNER JOIN appfd_alias ON (appfd_aliasplace.alias_id = appfd_alias.id) WHERE NOT (appfd_place.point IS NULL) AND appfd_alias.alias = ANY(names);
    most_common_country_code := country_code_rankings[0];
    firstchars := (SELECT ARRAY(SELECT left(name, 1) FROM unnest(names) AS name));
    RETURN QUERY
        SELECT id, admin_level, country_code, indexOf(country_code_rankings, country_code), (SELECT target || '--' || ldist FROM (SELECT target, levenshtein_less_equal(target, appfd_place.name, 2) AS ldist FROM unnest(names::TEXT[]) AS target ORDER BY ldist LIMIT 1) as s1)::text, name, NULL::varchar, population, point
        FROM appfd_place
        WHERE country_code = most_common_country_code
        AND NOT (appfd_place.point IS NULL)
